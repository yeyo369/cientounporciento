...
$(document).ready(function() {
  ...
  $(window).scroll(function (event) {
	...
	/*if($('#logo101').height() > 80){
		$('#logo101').height(function (index,height) {
    return (height - z);
});
	}*/
	
	if (y <= 1) {
		$('#logo101').height(50 + 'vh');
	}
	
    // whether that's below the form
    if (y > $('#focus').position().top) {
      // if so, ad the fixed class
	  $('#logo101').height(10 + 'vh');
	  $('#main-nav').addClass('aparecer');
	  $('#main-nav').addClass('fixed');
    } else {
      // otherwise remove it
	  $('#main-nav').removeClass('aparecer');
